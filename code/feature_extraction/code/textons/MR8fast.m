...
%I replaced anigauss with anigauss_mex because anigauss is a script.
...
for j=0:2,
    for k=0:5,
        ...
        if (obtainFilterNorm)
            ...
            im1 = s2 .* anigauss_mex(a, s1, s2, phi, 0, 1);
            im2 = (s2*s2) .* anigauss_mex(a, s1, s2, phi, 0, 2);
        ...
        im1 = n1 .* anigauss_mex(in, s1, s2, phi, 0, 1);
        im2 = n2 .* anigauss_mex(in, s1, s2, phi, 0, 2);
...
if (obtainFilterNorm)
    im1 = anigauss_mex(a, sigma, sigma, 0.0, 2, 0);
    im2 = anigauss_mex(a, sigma, sigma, 0.0, 0, 2);
    im1 = (s2*s2) .* (im1+im2);
    im2 = anigauss_mex(a, sigma, sigma);
    n1 = 1.0/sum(sum(abs(im1)));
...
ims{i} = n1 .* (im1+im2);
i=i+1;
ims{i} = n2 .* anigauss_mex(in, sigma, sigma);
